...
class StudentInfoModelImpl(private val context: Context):StudentInfoModel {
    private val apiInterface: CourseApiInterface = RetrofitClient.getClient().create(CourseApiInterface::class.java)

    override fun getSingleStudent(id: Int, token: String, callback: DataFetchCallback<StudentResponse>)
    {
        ...
        val call = token?.let { apiInterface.getSingleStudent(context.getString(R.string.api_key), it, id) }
